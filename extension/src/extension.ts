...
const TOOLS: Record<string, (params: Record<string, unknown>) => Promise<unknown>> = {
  ...
  start_solo_mode: async (params: Record<string, unknown>) => {
    try {
      await vscode.commands.executeCommand('trae.solo.mode.toggle');
  ...
  send_to_solo_chat: async (params: Record<string, unknown>) => {
  ...
  open_mcp_config: async (params: Record<string, unknown>) => {
    const scope = (params.scope as string) || 'user'; // 'user' or 'workspaceFolder'
    try {
  ...
  list_mcp_servers: async (params: Record<string, unknown>) => {
    try {
      const configs = [
      ...
      const allServers: Record<string, { name: string; scope: string; config: Record<string, unknown> }> = {};
      for (const cfg of configs) {
        try {
          ...
          for (const [name, serverCfg] of Object.entries(servers)) {
            allServers[name] = { name, scope: cfg.scope, config: serverCfg as Record<string, unknown> };
          }
        } catch {
  ...
  add_mcp_server: async (params: Record<string, unknown>) => {
    const name = params.name as string;
    const command = params.command as string;
    const args = (params.args as string[]) || [];
    if (!name || !command) throw new Error('Missing required parameters: name, command');
    try {
  ...
  open_deploy_panel: async (params: Record<string, unknown>) => {
    const deployCommands = [
      'icube.common.commands.tooling.deployToRemote',
      'trae.solo.guide.showSoloBuild',
      'icube.ai.focus-to-integration-view',
    ];
    const results: Array<{ command: string; success: boolean; result?: unknown; error?: string }> = [];
  ...
  open_browser_tool: async (params: Record<string, unknown>) => {
    const url = params.url as string | undefined;
    const action = (params.action as string) || 'navigate'; // navigate | screenshot | listTabs
    ...
    const browserAction = params.browserAction as string | undefined;
    const toolParams: Record<string, unknown> = {};
    if (url) toolParams.url = url;
    if (browserAction) toolParams.action = browserAction;

    try {
      const r = await vscode.commands.executeCommand(
        'icube.common.commands.tooling.browserUseTool',
        { action: browserAction || 'navigate', url, ...params }
      );
      return {
        success: true,
  ...
  get_trajectory_state: async (params: Record<string, unknown>) => {
    ...
    const specFiles: Array<{ path: string; exists: boolean; content?: string }> = [];
    ...
    try {
      ...
      const errors = allDiags.flatMap(([uri, diags]) =>
        diags.filter(d => d.severity === vscode.DiagnosticSeverity.Error).map(d => ({
          file: uri.fsPath,
          message: d.message,
  ...
  call_mcp_server: async (params: Record<string, unknown>) => {
    const serverName = params.server as string;
    const method = (params.method as string) || 'tools/call';
    const toolName = params.tool as string;
    const toolArgs = (params.arguments as Record<string, unknown>) || {};

    if (!serverName) throw new Error('Missing required parameter: server');
    if (!toolName && method === 'tools/call') throw new Error('Missing required parameter: tool');
    ...
    for (const cfgPath of configs) {
      try {
        ...
        const config = JSON.parse(content);
        if (config.mcpServers && (config.mcpServers as Record<string, unknown>)[serverName]) {
          serverConfig = (config.mcpServers as Record<string, { command: string; args: string[]; env?: Record<string, string> }>)[serverName];
          configPath = cfgPath;
          break;
        }
      } catch {
    ...
    return new Promise((resolve) => {
      const proc = spawn(serverConfig!.command, serverConfig!.args, { env, stdio: ['pipe', 'pipe', 'pipe'] });
      let stdout = '';
      let stderr = '';
  ...
  github_search_repos: async (params: Record<string, unknown>) => {
    ...
    return new Promise((resolve) => {
      ...
      proc.on('close', (code) => {
        if (code !== 0) { resolve({ success: false, error: stderr || `gh exited ${code}` }); return; }
  ...
  github_list_issues: async (params: Record<string, unknown>) => {
    ...
    return new Promise((resolve) => {
      ...
      proc.on('close', (code) => {
        if (code !== 0) { resolve({ success: false, error: stderr || `gh exited ${code}` }); return; }
        try { resolve({ success: true, issues: JSON.parse(stdout), count: JSON.parse(stdout).length }); }
        catch { resolve({ success: false, error: 'JSON parse failed' }); }
      });
      proc.on('error', (e) => resolve({ success: false, error: e.message }));
  ...
  github_get_repo: async (params: Record<string, unknown>) => {
    ...
    return new Promise((resolve) => {
      ...
      proc.on('close', (code) => {
        if (code !== 0) { resolve({ success: false, error: stderr || `gh exited ${code}` }); return; }
        try { resolve({ success: true, ...JSON.parse(stdout) }); }
        catch { resolve({ success: false, error: 'JSON parse failed' }); }
      });
      proc.on('error', (e) => resolve({ success: false, error: e.message }));
      setTimeout(() => { proc.kill(); resolve({ success: false, error: 'Timeout' }); }, 10000);
